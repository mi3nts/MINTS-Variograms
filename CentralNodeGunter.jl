...
data_frame_pm_updated = groupby(data_frame_pm_updated, "date")

# data_frame_pm_updated = CSV.read("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/monthly/dfUpdatedMonthly.csv", DataFrame)
...
# end
# println(data_frame_pm_updated_rolling_mean)
...
println(names(df_range))
CSV.write("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/monthly/dfRangeMonthly.csv", df_range)
...
# df_range = CSV.read("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/monthly/dfRangeMonthly.csv", DataFrame)
td= 900
ts_wind = 2
ts_tph = 10
...
df_wind_list = []
for i in 1:31
    wd_rolling_mean = RollingFunctions.rolling(mean,df_wind[i].windDirectionTrue,Int(td/ts_wind))
    ws_rolling_mean = RollingFunctions.rolling(mean,df_wind[i].windSpeedMetersPerSecond,Int(td/ts_wind))
    rolling_time_wind = collect(df_wind[i].dateTime[1]+Minute(15):Second(ts_wind):df_wind[i].dateTime[end]+Second(ts_wind))
    try
        df_wind_avg = DataFrame(RollingTime = rolling_time_wind,
                                MeanWindSpeed = ws_rolling_mean,
                                MeanWindDirection = wd_rolling_mean)

        push!(df_wind_list, df_wind_avg)
    catch
        println("failed on " * string(i))
    end
end
df_wind_combined = vcat(df_wind_list...,cols=:union)

df_tph_list = []
for i in 1:31
    temp_rolling_mean = rolling(mean,df_tph[i].temperature,Int(td/ts_tph))
    press_rolling_mean = rolling(mean,df_tph[i].pressure,Int(td/ts_tph))
    hum_rolling_mean = rolling(mean,df_tph[i].humidity,Int(td/ts_tph))
    rolling_time_tph = collect(df_tph[i].dateTime[1]+Minute(15):Second(ts_tph):df_tph[i].dateTime[end]+Second(ts_tph))
    ...
    try
        ...
        push!(df_tph_list, df_tph_avg)
    catch
        println("failed on" * string(i))
    end
end
df_tph_combined = vcat(df_tph_list...,cols=:union)


df_range_wind_tph_var = sort!(outerjoin(outerjoin(df_range,df_wind_combined,on = :RollingTime),df_tph_combined,on = :RollingTime),[:RollingTime])
...
#CSV.write("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/monthly/monthlyWind_TPH_Range.csv",df_range_wind_tph_var)
...
df_24 = data_frame_pm_updated[24]
select!(df_24, Not("date"))
df_17 = data_frame_pm_updated[17]
df_31 = data_frame_pm_updated[31]
df_updated_24 = DataFrames.rename!(df_24,["dateTime", "pc0.1","pc0.3","pc0.5","pc1.0","pc2.5","pc5.0","pc10.0","pm0.1","pm0.3","pm0.5","pm1.0","pm2.5","pm5.0","pm10.0"])
df_updated_17 = DataFrames.rename!(df_17,["dateTime", "pc0.1","pc0.3","pc0.5","pc1.0","pc2.5","pc5.0","pc10.0","pm0.1","pm0.3","pm0.5","pm1.0","pm2.5","pm5.0","pm10.0"])
df_updated_31 = DataFrames.rename!(df_31,["dateTime", "pc0.1","pc0.3","pc0.5","pc1.0","pc2.5","pc5.0","pc10.0","pm0.1","pm0.3","pm0.5","pm1.0","pm2.5","pm5.0","pm10.0"])

function generate_rolling_df(df, ts)
    data_frame_pm_updated_rolling_mean = DataFrame()
    data_frame_pm_updated_rolling_mean.RollingTime = ts
    for i in names(df)[2:end]
        data_frame_pm_updated_rolling_mean[!,i] = RollingFunctions.rolling(mean,df[!,i],Int(900))
    end

    data_frame_pm_updated_rolling_mean = DataFrames.rename!(data_frame_pm_updated_rolling_mean,["dateTime", "pc0.1","pc0.3","pc0.5","pc1.0","pc2.5","pc5.0","pc10.0","pm0.1","pm0.3","pm0.5","pm1.0","pm2.5","pm5.0","pm10.0"])
    return data_frame_pm_updated_rolling_mean
end

ts_24 = collect(df_24.dateTime[1]+Minute(15):Second(1):df_24.dateTime[end] + Second(1))
ts_17 = collect(df_17.dateTime[1]+Minute(15):Second(1):df_17.dateTime[end] + Second(1))
ts_31 = collect(df_31.dateTime[1]+Minute(15):Second(1):df_31.dateTime[end] + Second(1))
df_24_mean = generate_rolling_df(df_24, ts_24)
df_17_mean = generate_rolling_df(df_17, ts_17)
df_31_mean = generate_rolling_df(df_31, ts_31)
...
vec_df_pm_24 = group_by_dates(df_updated_24)
vec_df_pm_17 = group_by_dates(df_updated_17)
vec_df_pm_31 = group_by_dates(df_updated_31)

vec_df_pm_mean_24 = group_by_dates(df_24_mean)
vec_df_pm_mean_17 = group_by_dates(df_17_mean)
vec_df_pm_mean_31 = group_by_dates(df_31_mean)
...
gr()
tm_ticks = range(Time(df_24.dateTime[1]),Time(df_24.dateTime[end]),step = Hour(3))
...
for i in 1:3
    ...
    Plots.scatter(Time.(df.dateTime),df[!,"pm0.1"], xlabel = "01-" * jan_date * "-2023",
    ...
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm0.1.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm0.3"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations"*pm_unit, label = dict_plot_pm["pm0.3"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm0.3.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm0.5"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations"*pm_unit, label = dict_plot_pm["pm0.5"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm0.5.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm1.0"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations"*pm_unit, label = dict_plot_pm["pm1.0"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm1.0.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm2.5"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations"*pm_unit, label = dict_plot_pm["pm2.5"], 
        legend = :right,  legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm2.5.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm5.0"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations"*pm_unit, label = dict_plot_pm["pm5.0"], 
        legend = :right,  legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm5.0.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm10.0"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations "*pm_unit, label = dict_plot_pm["pm10.0"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm10.0.png")
end


for i in 1:3
    if i == 1
        df = vec_df_pm_mean_24[1]
        jan_date = "24"
    elseif i == 2
        df = vec_df_pm_mean_17[1]
        jan_date = "17"
    elseif i == 3
        df = vec_df_pm_mean_31[1]
        jan_date = "31"
    end
    Plots.scatter(Time.(df.dateTime),df[!,"pm0.1"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ...
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm0.1mean.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm0.3"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations Rolling Mean"*pm_unit, label = dict_plot_pm["pm0.3"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm0.3mean.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm0.5"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations Rolling Mean"*pm_unit, label = dict_plot_pm["pm0.5"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm0.5mean.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm1.0"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations Rolling Mean"*pm_unit, label = dict_plot_pm["pm1.0"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm1.0mean.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm2.5"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations Rolling Mean"*pm_unit, label = dict_plot_pm["pm2.5"], 
        legend = :right,  legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm2.5mean.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm5.0"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations Rolling Mean"*pm_unit, label = dict_plot_pm["pm5.0"], 
        legend = :right,  legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm5.0mean.png")
    Plots.scatter(Time.(df.dateTime),df[!,"pm10.0"], xlabel = "01-" * jan_date * "-2023",
        xticks = (tm_ticks,ticks),markerstrokewidth=0,markersize=3,
        ylabel = "PM concentrations Rolling Mean"*pm_unit, label = dict_plot_pm["pm10.0"], 
        legend = :right, legendfontsize=10, xrotation = 30)
    png("C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/plots/jan" * jan_date * "/pm10.0mean.png")
end
