...
path_to_params = "C:/Users/va648/Downloads/VSCode/MINTS-LoRa-Variograms/firmware/data/" #path for csv files
df_pm_list = []
for i in 1:114
...
function data_cleaning(data_frame)
    ...
    col_symbols = [:pm0_1,:pm0_3,:pm1_0,:pm2_5,:pm5_0, :pm10_0]
    ...
    #         end 
    #     end
    #     if (typeof(data_frame[!,:pm0_5][i]) == String31 || typeof(data_frame[!,:pm0_5][i] == String)) 
    #         try
    ...
    data_frame = DataFrames.combine(DataFrames.groupby(data_frame, :dateTime), [:pm0_1,:pm0_3,:pm1_0,:pm2_5,:pm5_0, :pm10_0] .=> mean) # what exactly does this do
...
CSV.write(path_to_params*"Joppa_2022.csv", data_frame_pm)

ts = collect(data_frame_pm.dateTime[1]+Minute(15):Second(1):data_frame_pm.dateTime[end] + Second(1))
data_frame_pm_rolling_mean = DataFrame(RollingTime, pm0_1_mean, pm0_3_mean, pm1_0_mean, pm2_5_mean, pm5_0_mean, pm10_0_mean)
print(data_frame_pm_rolling_mean)
data_frame_pm_rolling_mean.RollingTime = ts

for i in names(data_frame_pm)[2:end]
    data_frame_pm_rolling_mean[!,i] = RollingFunctions.rolling(mean,data_frame_pm[!,i],Int(900))
end

CSV.write(path_to_params*"PMRollingMean.csv",data_frame_pm_rolling_mean)


#########changes from here up############
